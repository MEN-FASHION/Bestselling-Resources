...
- **分享类目预览（招品回品 / BESTSELLER）**：超管在招品/BESTSELLER 面板工具栏点「复制分类」按钮，选择类目后复制其**前台分享链接**（形如 `trends.html?zone=recruit&cat=类目名`），发给用户。用户打开该链接时，若未登录，仅能浏览**该类目已发布任务的缩略图预览**（点击任意图片提示登录，不展示高清大图/SPU/商家信息）；登录后自动定位到该类目并恢复完整功能。该功能通过「匿名读已发布任务」数据库策略 + 图片服务对未登录放行实现（`share_preview_anon.sql`），仅允许已发布任务对匿名可见，草稿/后台敏感数据仍受保护；其余类目保持强防爬。
