...
- **上传**：选择/新建分类 → 拖拽多张图片 → 点“上传所选”。图片经 Worker 写入 R2，清单记录进数据库，前台立即可见。**上传自动去重**：与库中同名图片会被跳过，不会重复上传。**上传自动压缩**：大图在浏览器端自动压到最长边约 1600px 的高质量 JPEG（小图/已轻量图不处理），显著减小体积，加快前台（尤其是手机端）加载。
