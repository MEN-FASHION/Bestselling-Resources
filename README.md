...
**营销日历（前台第一个菜单项）**
- 顶部菜单新增**「营销日历」**，为第一个菜单项，点击进入独立的 `marketing.html` 页面。页面顶部为**营销时间线**，时间线上分布**时间节点**，每个节点对应下方一张 **3:4 竖版**展示卡片；卡片为**展示图**，点击跳转到该节点关联的**趋势文章链接**。
...
- **营销日历管理（仅超管可编辑）**：超管权限面板新增「营销日历」区块，两个子功能：**时间线节点管理**——新增/编辑/删除时间节点（标题 + 时间），节点决定前台时间线点位；**趋势文章管理**——为某个节点发布文章（标题、外部文章链接、摘要、3:4 封面图、发布/下架开关），发布后前台展示为可点击卡片；每篇文章支持「微信分享」（生成专属二维码）。营销日历封面图走受控通道存 R2（`marketing/`），前台登录后可看、不可直接下载。
