...
- **超管权限**：侧边栏「超管权限」为超级管理员专属菜单（普通管理员不可见），收拢全部超管专属功能，内含四大功能区块：**公告管理**（编辑/发布/下架/删除公告，前台登录后展示近一个月未读公告）、**权限管理**（设置用户角色，并按类目分配管理员可管理的类目权限）、**前台访问设置**（公开浏览开关、各专区各类目前台可见性开关）、**数据导出**（按前台筛选条件导出 Excel，含图片与分类标签）。前台类目展示顺序仅超管可在「标签管理 → 展示顺序」处调整。
