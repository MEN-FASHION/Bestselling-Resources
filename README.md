...
- **营销日历管理（超管）**：超管权限面板新增「营销日历」区块，两个子功能：**时间线节点管理**——新增/编辑/删除时间节点（标题 + 时间），节点决定前台时间线点位；**趋势文章管理**——为某个节点发布文章（标题、外部文章链接、摘要、3:4 封面图、发布/下架开关），发布后前台展示为可点击卡片；每篇文章支持「微信分享」（生成专属二维码）。营销日历封面图走受控通道存 R2（`marketing/`），前台登录后可看、不可直接下载。
