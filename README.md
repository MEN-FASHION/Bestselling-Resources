...
- **后台审核标记（招品回品 / BESTSELLER）**：两张任务卡片图片右下角提供「IP / 品牌 / 类目错放 / 无需回品」四个勾选框，勾选即打标记（仅后台可见，前台不展示）。面板顶部有**标记统计图表**（可按类目筛选，展示各标记的数量与占比）；工具栏提供**按标记筛选**；**导出 Excel** 会包含「类目 / 标记 / IP / 品牌 / 类目错放 / 无需回品」字段。新库需执行 `tools/setup.sql` 或单独执行 `add_task_flags.sql` 以创建 4 个标记字段（`flag_ip/flag_brand/flag_cat_mismatch/flag_no_refill`）。
...
- **上传（在「图片管理 → 智能打标」弹窗内）**：打开图片管理 → 点「智能打标」进入弹窗，在右侧「上传图片」区选择类目 → 拖拽/点击多选图片 → 点「上传」。图片经 Worker 写入 R2，清单记录进数据库，前台立即可见。**上传自动去重**：与库中同名图片会被跳过，不会重复上传。**上传自动压缩**：大图在浏览器端自动压到最长边约 1600px 的高质量 JPEG（小图/已轻量图不处理），显著减小体积，加快前台（尤其是手机端）加载。
...
## 前台登录页与分享预览（2026-09-16 更新）

- **登录按钮文案**：登录页主按钮由「进入图鉴」改为「进入网站」。
- **登录上传横幅**：前台主导航下方新增一条蓝色渐变横幅，未登录时显示「登录并上传货品，可获得流量加权，持续触达更多浏览」，点击「立即登录」跳转登录；登录后自动隐藏。
- **无图类目隐藏**：招品回品 / BESTSELLER 前台左侧类目菜单只展示「已有已发布任务」的类目，无内容的类目不再出现在菜单栏。
- **分享链接定位**：后台「复制分类」按钮按左侧类目菜单当前选中类目生成 `trends.html?zone=recruit|bestseller&cat=类目名` 分享链接；打开后自动定位到对应专区与类目。未登录进入分享链接：显示该类目已发布任务缩略图，点图提示登录、不展示大图/SPU/商家信息；登录后恢复完整功能。
  - 若打开分享链接未定位到类目，多为浏览器缓存旧版 JS 所致，请前台强刷（Ctrl+F5）或清除缓存。

> 分享类目缩略图预览依赖匿名读策略：请在 Supabase SQL Editor 执行 `share_preview_anon.sql`（幂等），开启「已发布且未删除」任务对匿名开放读取。
