...
- **超管权限**：侧边栏「超管权限」为超级管理员专属菜单（普通管理员不可见），收拢全部超管专属功能，内含五大功能区块：**公告管理**（编辑/发布/下架/删除公告，前台登录后展示近一个月未读公告）、**权限管理**（设置用户角色、按类目分配管理员可管理的类目权限、为每个用户单独配置「用户级可见专区」，并提供「用户权限分配表」总览所有用户的角色/类目/专区授权）、**前台访问设置**（公开浏览开关、各专区各类目前台可见性开关）、**数据导出**（按前台筛选条件导出 Excel，含图片与分类标签）、**存量图片压缩池**（左池＝已压缩 ≤200KB，右池＝待压缩 >200KB；可在右池单选/多选/全选批量压缩，压缩自动转为 WebP 并压到 200KB 以下，完成后自动跳入左池；单张可点「预览」检查清晰度、点「转WEB」强制转 WebP；带进度显示）。前台类目展示顺序仅超管可在「标签管理 → 展示顺序」处调整。
